# TENHLE SOUBOR MA BYT O ADRESAR VYS
# NAHRAL A ADDNUL JSEM JEJ SEM, PROTOZE GIT COMMIT MI NEPISE SUDOKU.PY JAKO NOVOU ZMENU, TAK PROSTE ZALOHA
...
def solve(problem):
  ...
  def last_show(typ, data):
    if typ == 'normal': last_show_two(data)
      ...
      for i in range(len(data)): print(' --- solution no. ' + str(i+1)); last_show_two(data.get())
  def last_show_two(board):
  ...
  if validity == 'unfinished board':    # if there is some job left
    ...
    if   result.length() == 1: sudoku = result.get() # if there is just one matrix as a possible solution, load it into the original board
    elif result.length() == 0: print("Something is wrong (Sheldon's personal seat is Penny-butted)")
  ...
  if special_end:                       # alternative returning value
    ...
    last_show('normal', sudoku); return sudoku
# *** the end
